...
// JSON body that were generated by the chat→responses conversion path. Those
// IDs used a "chatcmpl-..._msg_0" format, but the Responses API requires IDs
// to start with "msg_". Only chatcmpl- prefixed IDs are rewritten; all other
// IDs (msg_, rs_, etc.) are left untouched.
func normalizeResponsesInputIDsJSON(data []byte) ([]byte, error) {
	...
	for i := range input {
		...
		if !ok || id == "" || !strings.HasPrefix(id, "chatcmpl-") {
			continue
		}
		item["id"] = "msg_" + id
		input[i] = item
...
// chat→responses conversion path. Those IDs used a "chatcmpl-..._msg_0" format,
// but the Responses API requires IDs to start with "msg_". Only chatcmpl-
// prefixed IDs are rewritten; all other IDs (msg_, rs_, etc.) are left
// untouched so users can switch between chat and responses models.
